...
client = MongoClient()
db = client.mydb
all_collection=["StudentOfferLetters.files"]
        ...
        if count !=0:
                if not os.path.exists(path):
                        ...
                        for document in cursor:
                                ...
                                try:
                                        db.students.update({'placementDetails.offerLetter':str(document['_id'])},{'$set':{'placementDetails.offerLetter':filenamenew}})
                                except:
                                        print("Can not update students collection"+str(document['_id']))
                                try:
                                        os.system("mongofiles -d eKaushal get '"+filenameold+"' --prefix '"+collection.split('.')[0]+"'")
                                except:
