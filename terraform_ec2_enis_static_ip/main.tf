...
resource "aws_instance" "ec2_node1" {
  ...
  network_interface {
    network_interface_id = aws_network_interface.ec2_primary_1.id
    device_index         = 0
  }
  network_interface {
    network_interface_id = aws_network_interface.ec2_mgmt_1.id
    device_index         = 1
  }
  root_block_device {
    encrypted   = "true"
...
resource "aws_network_interface" "ec2_primary_1" {
  subnet_id       = "subnet-1e21093f"
  private_ips     = ["172.31.80.17"]
}

resource "aws_network_interface" "ec2_mgmt_1" {
  subnet_id       = "subnet-1e21093f"
