...
TFLINT_VERSION=${TFLINT_VERSION:-0.9.3}
TF_VERSION=${TF_VERSION:-0.12.9}
...
for dir in $(find modules ! -path modules -type d); do
  ...
  if [[ $dir =~ _lower_env_only_pls ]]; then
    printf  "Skipping check for modules not used in production for the time being... \n\n"
  else
...
docker run --rm --entrypoint terragrunt -w /apps/environments -v $(pwd):/apps \
